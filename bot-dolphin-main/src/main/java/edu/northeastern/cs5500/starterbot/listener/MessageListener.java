...
import edu.northeastern.cs5500.starterbot.command.EditEventButtonHandler;
...
public class MessageListener extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@Nonnull SlashCommandInteractionEvent event) {
        ...
        // Skip verification for the "/verify" command
        if (!commandName.equalsIgnoreCase("verify")) {
            if (!eventUserController.isUserVerified(discordUserId)) {
                event.reply(
                                "You must verify your NEU email address (/verify) to access the event bot features.")
                        .setEphemeral(true)
                        .queue();
                return; // Stop processing the command if the user is not verified
            }
        }
    ...
    public void onModalInteraction(@Nonnull ModalInteractionEvent event) {
        ...
        if (modalId.startsWith("edit_event_")) {
            String eventId = modalId.substring("edit_event_".length());
            buttons.values().stream()
                    .map(Provider::get)
                    .filter(handler -> handler instanceof EditEventButtonHandler)
                    .findFirst()
                    .ifPresent(
                            handler ->
                                    ((EditEventButtonHandler) handler)
                                            .handleEditEventModalInteraction(event, eventId));
        }
    }
}
